...
test("render product backlog", function() {
	...
	equals(options[0].text, '(2) Preencher Dados Gerais do Fornecedor')
	equals(options[1].text, '(2) Buscar Fornecedor')
	equals(options[2].text, '(5) Cadastrar Produtos do Fornecedor')
	equals(options[3].text, '(2) Visualizar Perfil da Universidade')
	equals(options[35].text, '(3) Moderar F\u00f3rum')
	
	// optgroups
	var groups = get_groups()
	equals(groups.length, 11)
	equals(groups[0].label, 'Fornecedores')
	equals(groups[1].label, 'Universidades')
	equals(groups[2].label, 'Portal')
	equals(groups[10].label, 'F\u00f3rum de Discuss\u00e3o')
});
...
test("add", function() {
	...
	var options = get_options()
	equals(options.length, 35)
	equals(options[0].text, '(2) Buscar Fornecedor')
...
test("remove", function() {
	...
	var options = get_options()
	equals(options.length, 36)
	equals(options[0].text, '(3) Buscar Empresa de Arquitetura')
